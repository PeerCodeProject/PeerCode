...
export class DockerService extends BaseObservable<DockerPortListener> {
  ...
  async runDockerLocallyAndShare(workspacePath: string, session: Session): Promise<void> {
  ...
  async runProject(workspacePath: string, roomName: string): Promise<string> {
  ...
  private static getImageName(roomName: string): string {
    return "peercode-" + roomName.toLowerCase();
  }

  private static getLogPath(workspacePath: string, containerId: string): string {
  ...
  public runDockerRemote(session: Session): void {
    const provider = session.provider.getProvider();
    provider.emit("runDockerRemote", [session.getUsername()]);
  }

  public listenToDockerRun(provider: Observable<string>, session: Session): void {
