...
export class FileStore {
  ...
  saveFile(peerFile: SharedPeerFile): void {
...
export class SharedPeerFile {
  constructor(
    public readonly filenameKey: string,
...
export default class FileShareManager implements IShareLocalToRemote, RemoteFileListener {
  ...
  private async createAndSaveSharedFile(
    fileKey: string,
    uri: vscode.Uri,
    editorChannel: EditorChannel,
  ): Promise<SharedPeerFile> {
  ...
  async onAddRemoteFile(fileKey: string, editorChannel: EditorChannel): Promise<void> {
    ...
    if (sharedFile) {
      console.warn(
        "FileShareManager onAddRemoteFile: shared file is already exists for key: " + fileKey,
      );
      return;
    }
  ...
  onDeleteRemoteFile(filename: string): void {
    ...
    if (!sharedFile) {
      console.warn(
        "FileShareManager onDeleteRemoteFile: shared file is not exists for key: " + filename,
      );
      return;
    }
