...
export class DocumentManager implements IDocumentManager {
  constructor(
    ...
    private sharer: IShareLocalToRemote,
  ) {
  ...
  public onChangeTextDocument(event: vscode.TextDocumentChangeEvent): void {
    if (event.contentChanges.length === 0) {
      console.log(
        "no content changes: reason" + event.reason + ", doc name:" + event.document.fileName,
      );
      return;
    }
    console.log(
      "Handling change event: " + event.contentChanges.length,
      ", doc name:" + event.document.fileName,
    );

    const sharedFile = this.fileStore.getSharedFileByUri(event.document.uri);
    if (!sharedFile) {
      console.error(
        "onChangeTextDocument: No shared file found for document: " + event.document.uri,
      );
      return;
    }
  ...
  private onSaveDocument(event: vscode.TextDocumentWillSaveEvent): void {
  ...
  private onCreateFiles(event: vscode.FileCreateEvent): void {
  ...
  private onRenameFiles(event: vscode.FileRenameEvent): void {
