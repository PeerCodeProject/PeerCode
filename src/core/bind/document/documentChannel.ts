...
export class YDocumentChannel
  extends BaseObservable<DocumentChannelListener>
  implements DocumentChannel
{
  private currentText = "";
  ...
  private initObservers(): void {
    ...
    const saveObserver = (event: Y.YArrayEvent<string>, transaction: Y.Transaction): void => {
      if (transaction.origin === this.currentUsername) {
        return;
      }
  ...
  private async onRemoteTextChanged(event: Y.YTextEvent): Promise<void> {
    ...
    for (const delta of event.changes.delta) {
      if (delta.retain) {
        ...
        const textDelete = new TextChange(
        ...
        const textInsert = new TextChange(
  ...
  private onRemoteSave(_event: Y.YArrayEvent<string>): void {
  ...
  private sendChangeInTransaction(change: TextChange, startIndex: number, endIndex: number): void {
    this.doc.transact(() => {
