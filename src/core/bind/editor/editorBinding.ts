...
class UserDecorator {
  constructor(
    public selectionDecoration: vscode.TextEditorDecorationType,
...
class UserDecoratorStore {
  ...
  static getInstance(): UserDecoratorStore {
  ...
  getColor(peer: string): UserDecorator {
...
export default class EditorBinding implements EditorChannelListener {
  ...
  constructor(public editorChannel: EditorChannel) {
    editorChannel.addListener(this);
  }

  onSelectionsChangedForPeer(
    peername: string,
    ...
    for (const selection of selections) {
      if (selection.isCursor) {
        cursorRanges.push(
          new vscode.Range(getVSCodePosition(selection.start), getVSCodePosition(selection.end)),
        );
      } else {
  ...
  updateSelections(selections: readonly vscode.Selection[]): void {
